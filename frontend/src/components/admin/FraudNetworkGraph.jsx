...
export default function FraudNetworkGraph({ targetCustomerId = null, onSelectCustomer }) {
  ...
  const graphData = useMemo(() => {
    ...
    if (filterMode === 'FLAGGED') {
      ...
      nodes = nodes.filter(n => n.isFlagged || links.some(l => (flaggedIds.has(l.source?.id || l.source) && (l.target?.id || l.target) === n.id)));
      const activeIds = new Set(nodes.map(n => n.id));
      links = links.filter(l => activeIds.has(l.source?.id || l.source) && activeIds.has(l.target?.id || l.target));
  ...
  // Custom Node Canvas Painting for Light Theme
  const paintNode = useCallback((node, ctx, globalScale) => {
    ...
    try {
      // 1. Draw outer halo for flagged nodes / selected node
      if (isMule || isSelected || isHovered) {
        const pulseTime = Date.now() / 400;
        const pulseRadius = radius + (isMule ? 4 + Math.sin(pulseTime) * 1.5 : 2.5);

        if (Number.isFinite(pulseRadius) && pulseRadius > radius * 0.8) {
          const glowGrad = ctx.createRadialGradient(x, y, Math.max(1, radius * 0.8), x, y, pulseRadius);
          if (isMule) {
      ...
      // 2. Node Core with Light Theme Radial Gradient
      const innerR = Math.max(0.5, radius * 0.1);
      const outerR = Math.max(innerR + 1, radius);
      const coreGrad = ctx.createRadialGradient(x - radius * 0.3, y - radius * 0.3, innerR, x, y, outerR);
      
      if (node.maxRiskScore >= 85 || isMule) {
        coreGrad.addColorStop(0, '#f87171');
        coreGrad.addColorStop(0.6, '#dc2626');
        coreGrad.addColorStop(1, '#991b1b');
      } else if (node.maxRiskScore >= 60) {
        ...
        coreGrad.addColorStop(0.6, '#059669');
      ...
      // 3. Node Border Ring (Crisp white border)
    ...
    // 4. Label rendering with clean light-theme pill
    try {
      if (globalScale > 1.3 || isHovered || isSelected || isMule) {
        ...
        if (label) {
          ...
          if (Number.isFinite(textWidth)) {
            // White backdrop pill with subtle border
            ...
            ctx.fillStyle = isMule ? '#b91c1c' : (isSelected ? '#1d4ed8' : '#334155');
  ...
  // Particle color based on link risk
  const getParticleColor = useCallback((link) => {
    if (link.avgRisk >= 75 || link.severity === 'critical') return '#dc2626'; // Vivid Red
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
        <div className="p-3.5 rounded-xl bg-white border border-slate-200 shadow-card space-y-1">
          ...
          <p className="text-xl font-bold text-slate-900 font-mono">{graphData.nodes.length} <span className="text-xs font-normal text-slate-500">active</span></p>
        </div>
        ...
        <div className="p-3.5 rounded-xl bg-white border border-slate-200 shadow-card space-y-1">
          ...
          <p className="text-xl font-bold text-rose-700 font-mono">{flaggedNodesCount} <span className="text-xs font-normal text-slate-500">flagged</span></p>
        </div>
        ...
        <div className="p-3.5 rounded-xl bg-white border border-slate-200 shadow-card space-y-1">
          ...
          <p className="text-xl font-bold text-emerald-700 font-mono">{graphData.links.length} <span className="text-xs font-normal text-slate-500">edges</span></p>
        </div>

        <div className="p-3.5 rounded-xl bg-white border border-slate-200 shadow-card space-y-1">
          <div className="flex items-center justify-between text-slate-500 text-xs font-semibold uppercase">
            <span>In-Flight Volume</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 items-start">
        ...
        >
          ...
          <div className="absolute top-3 left-3 right-3 flex flex-wrap items-center justify-between gap-2 z-10 pointer-events-none">
            {/* Filter Toggle Pills */}
            <div className="flex items-center space-x-1 p-1 rounded-lg bg-white/95 backdrop-blur-md border border-slate-200 pointer-events-auto shadow-sm text-xs">
              {[
                { id: 'ALL', label: 'All Entities' },
                { id: 'FLAGGED', label: '🚨 Flagged Mules Only' },
            ...
            {/* Quick Actions HUD */}
            <div className="flex items-center space-x-1 p-1 rounded-lg bg-white/95 backdrop-blur-md border border-slate-200 pointer-events-auto shadow-sm">
              <button
                onClick={() => setIsOrbiting(!isOrbiting)}
                className={`p-1.5 rounded-md text-xs font-semibold flex items-center space-x-1 transition ${
                  isOrbiting ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'
                }`}
          ...
          {loading ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center space-y-2 text-xs text-blue-600 font-medium">
              ...
              <p>Simulating force-directed graph physics...</p>
            </div>
            ...
            <ForceGraph2D
              ...
              linkColor={(l) => l.avgRisk >= 75 ? 'rgba(220, 38, 38, 0.4)' : (l.avgRisk >= 50 ? 'rgba(217, 119, 6, 0.35)' : 'rgba(37, 99, 235, 0.25)')}
              // Glowing Animated Transforming Particle Packets Streaming Across Edges
              linkDirectionalParticles={3}
              linkDirectionalParticleSpeed={(l) => Math.max(0.006, Math.min(0.018, particleSpeed * (l.value > 10000 ? 1.4 : 1)))}
          ...
          <div className="absolute bottom-3 left-3 p-3 rounded-lg bg-white/95 backdrop-blur-md border border-slate-200 text-[11px] space-y-1.5 shadow-sm z-10 font-medium">
            <p className="font-bold text-slate-800">Particle Flow Legend:</p>
            <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-slate-600">
              <div className="flex items-center space-x-2">
                <span className="w-2.5 h-2.5 rounded-full bg-red-600" />
                <span>🚨 Mule Hub (Score &gt;80)</span>
              </div>
              <div className="flex items-center space-x-2">
                <span className="w-2.5 h-2.5 rounded-full bg-amber-500" />
                <span>⚡ Elevated Transfer</span>
              </div>
              <div className="flex items-center space-x-2">
        ...
        <div className="p-5 rounded-xl bg-white border border-slate-200 space-y-4 shadow-card">
          ...
          {selectedNode ? (
            <div className="space-y-4 text-xs animate-fade-in">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="p-3 rounded-lg bg-slate-50 border border-slate-200 space-y-0.5">
                  ...
                  <p className={`text-lg font-bold font-mono ${
                    selectedNode.maxRiskScore > 75 ? 'text-rose-700' :
                    selectedNode.maxRiskScore > 40 ? 'text-amber-700' : 'text-emerald-700'
                  }`}>
              ...
              {selectedNode.id?.startsWith('CUST-') && (
                <button
                  ...
                  className="w-full py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs flex items-center justify-center space-x-1.5 transition shadow-sm"
                >
            ...
            <div className="text-center py-16 space-y-2">
              ...
              <p className="text-xs text-slate-700 font-semibold">No Entity Selected</p>
