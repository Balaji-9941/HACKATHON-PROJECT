...
import { HeartPulse, Database, Cpu, Sparkles, Server, RefreshCw, AlertCircle } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
        ...
        <div className="p-3.5 rounded-lg bg-slate-50 border border-slate-200 space-y-1.5">
          <div className="flex items-center justify-between">
            ...
            <span className={`w-2.5 h-2.5 rounded-full ${isMongoHealthy ? 'bg-emerald-500' : 'bg-red-500'}`} />
          </div>
          <p className="text-[11px] text-slate-600 font-mono">
            State: <strong className={isMongoHealthy ? 'text-emerald-700' : 'text-red-700'}>{isMongoHealthy ? 'CONNECTED' : 'DISCONNECTED'}</strong>
          </p>
        </div>
        ...
        <div className="p-3.5 rounded-lg bg-slate-50 border border-slate-200 space-y-1.5">
          ...
          <p className="text-[11px] text-slate-600 font-mono">
            Circuit: <strong className={isMLHealthy ? 'text-blue-700' : 'text-amber-700'}>{isMLHealthy ? 'CLOSED (Active)' : 'OPEN (Tier 1 Fallback)'}</strong>
          </p>
        </div>
        ...
        <div className="p-3.5 rounded-lg bg-slate-50 border border-slate-200 space-y-1.5">
          ...
          <p className="text-[11px] text-slate-600 font-mono">
            Provider: <strong className="text-slate-800">{health?.aiNarrative?.provider || 'Template Fallback'}</strong>
          </p>
        </div>
