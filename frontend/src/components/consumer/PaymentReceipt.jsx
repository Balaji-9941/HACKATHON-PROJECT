...
import { CheckCircle2, ShieldCheck, ArrowRight, Clock, AlertCircle, Share2 } from 'lucide-react';
    ...
    <div className="space-y-4 animate-fade-in text-slate-900">
      ...
      <div className={`p-4 rounded-xl border ${riskStyle.bg} ${riskStyle.border} space-y-2`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <span className={`w-2.5 h-2.5 rounded-full ${
              transaction.totalRiskScore <= 30 ? 'bg-emerald-600' :
              transaction.totalRiskScore <= 50 ? 'bg-yellow-500' :
              transaction.totalRiskScore <= 70 ? 'bg-amber-600' : 'bg-red-600'
            }`} />
        ...
        <div className="pt-2 border-t border-slate-200/80 flex items-center justify-between text-[11px] text-slate-600">
          ...
          <span>Latency: <strong className="text-emerald-700 font-mono font-bold">{transaction.latencyMs || 12}ms</strong></span>
        </div>
      </div>
