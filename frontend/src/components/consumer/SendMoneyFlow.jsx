      ...
      try {
        ...
        const data = await fetchAPI('/transactions/pre-check', {
          ...
          body: JSON.stringify({
            ...
            location: activeCustomer.usualLocation || 'Bangalore, IN',
            deviceId: activeCustomer.knownDevices?.[0] || 'dev-pixel-8',
            deviceName: activeCustomer.knownDevices?.[0]?.includes('iphone') ? 'iPhone 15 Pro' : activeCustomer.knownDevices?.[0]?.includes('galaxy') ? 'Galaxy S24 Ultra' : 'Pixel 8 Pro',
